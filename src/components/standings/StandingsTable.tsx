  ...
  for (const method of settings.tiebreakOrder) {
    if (method === "DE") {
      if (tiedIds.length === 2) {
        ...
        if (!match || match.result === null) {
          results.push({
            ...
            reason: "La partida no tiene un resultado registrado.",
          });
        } else if (match.result === "draw") {
          results.push({
            label: TIEBREAK_INFO.DE.name,
            reason: "su partida enfrentada terminó en tablas.",
          });
        } else {
          // This shouldn't happen in an unresolved tie, but be safe
          results.push({
            label: TIEBREAK_INFO.DE.name,
            reason: "su partida enfrentada tiene resultado decisivo.",
          });
        }
      } else {
      ...
      results.push({
        ...
        reason:
          "quedan igualados al considerar el resultado directo y las tablas con piezas negras.",
      });
    } else if (method === "SB") {
      results.push({
        label: TIEBREAK_INFO.SB.name,
        reason:
          "tienen la misma puntuación considerando calidad de rivales vencidos.",
      });
      ...
      results.push({
        ...
        reason:
          "tienen la misma puntuación contra rivales clasificados (≥50% puntos).",
      });
    } else {
      // Unknown method
...
export function StandingsTable({ group, settings }: StandingsTableProps) {
  const [selectedTiebreak, setSelectedTiebreak] =
    useState<TiebreakMethod | null>(null);
  const entries = computeRankedStandings(group, settings);
  const selectedTiebreakInfo = selectedTiebreak
    ? TIEBREAK_INFO[selectedTiebreak]
    : null;

  // Only show tiebreak columns for methods that actually resolved at least one tie
  const usedTiebreaks = new Set(
    entries
      .map((e) => e.tiebreakUsed)
      .filter((m): m is TiebreakMethod => m !== null),
  );
  const tiebreakCols = settings.tiebreakOrder.filter((m) =>
    usedTiebreaks.has(m),
  );
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm table-fixed">
        ...
        <tbody>
          {entries.map((entry: StandingEntry) => {
            const name =
              nameMap.get(entry.participantId) ?? entry.participantId;
      ...
      {unresolvedTies.map((tiedEntries) => {
        ...
        const names = formatNameList(
          tiedIds.map((id) => nameMap.get(id) ?? id),
        );
        const reasons = explainUnresolvedTie(tiedIds, group, settings);
        return (
          <div
            key={tiedIds.join(",")}
            className="mt-3 rounded-md bg-[--surface-sunken] py-2 text-xs text-muted-foreground leading-snug space-y-1"
          >
            <p className="font-medium text-foreground">
              Empate sin resolver entre {names}
            </p>
            {reasons.map((r, i) => (
              <p key={i}>
                {r.label && (
                  <span className="text-chart-2 font-medium">{r.label}:</span>
                )}{" "}
                {r.reason}
              </p>
            ))}
